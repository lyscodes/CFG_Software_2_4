...
# for this api you only get 28 calls a month... I think that is fine for this project but to limit api calls I have created a hardcoded list to not waste calls whilst we practice
# need to figure out how to make it dynamic at a later point
def get_quote_by_mood():
    # result = requests.get('https://mood-based-quote-api.p.rapidapi.com/keyword', headers={'x-rapidapi-key': MOOD_API_KEY}).json()
    ...
    # to make this use api when ready uncomment first 2 lines and change hard_code_list below to result_list
